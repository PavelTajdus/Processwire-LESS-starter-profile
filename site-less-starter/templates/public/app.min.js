...
var ref=scrollParents(element,/auto|scroll/,!0);var scrollElement=ref[0];var clientHeight=scrollElement.clientHeight;var scrollHeight=scrollElement.scrollHeight;var scrollTop=scrollElement.scrollTop;var viewportTop=offsetPosition(element)[0]-scrollTop-offsetPosition(scrollElement)[0];var viewportDist=Math.min(clientHeight,viewportTop+scrollTop);var top=viewportTop-viewportDist;var dist=Math.min(element.offsetHeight+heightOffset+viewportDist,scrollHeight-(viewportTop+scrollTop),scrollHeight-clientHeight);return clamp(-1*top/dist)}
...
var getComponentName=cacheFunction(function(attribute){return startsWith(attribute,'uk-')||startsWith(attribute,'data-uk-')?camelize(attribute.replace('data-uk-','').replace('uk-','')):!1});var UIkit=function(options){this._init(options)};UIkit.util=util;UIkit.data='__uikit__';UIkit.prefix='uk-';UIkit.options={};UIkit.version='3.6.16';globalAPI(UIkit);hooksAPI(UIkit);stateAPI(UIkit);componentAPI(UIkit);instanceAPI(UIkit);function Core(UIkit){if(!inBrowser){return}
...
var ref$1=scrollParents(this.targets,/auto|scroll/,!0);var scrollElement=ref$1[0];var clientHeight=scrollElement.clientHeight;var scrollTop=scrollElement.scrollTop;var scrollHeight=scrollElement.scrollHeight;var max=scrollHeight-clientHeight;var active=!1;if(scrollTop===max){active=length-1}else{this.targets.every(function(el,i){if(offset(el).top-offset(getViewport(scrollElement)).top-this$1.offset<=0){active=i;return!0}});if(active===!1&&this.overflow){active=0}}
